...
def load_portfolio_history():
    """Lataa salkun historiatiedot tiedostosta."""
...
def save_portfolio_history(history):
    """Tallentaa salkun historiatiedot tiedostoon."""
...
def display_portfolio_summary(df, total_row, portfolio_name):
    ...
    st.subheader("Salkun arvon kehitys")
    history = load_portfolio_history()
    
    if portfolio_name in history and len(history[portfolio_name]) > 1:
        history_df = pd.DataFrame(history[portfolio_name])
        history_df.columns = ['Päivämäärä', 'Arvo']
        
        # Sijoita Arvo-sarakkeen arvot numeroiksi
        history_df['Arvo'] = history_df['Arvo'].astype(float)
        ...
        line_chart = alt.Chart(history_df).mark_line(point=True).encode(
            ...
            tooltip=['Päivämäärä', alt.Tooltip('Arvo', format='.2f')]
        ).properties(
            title="Salkun arvon kehitys"
        )
        st.altair_chart(line_chart, use_container_width=True)
    else:
        st.info("Ei tarpeeksi historiatietoa kaavion piirtämiseen. Tallenna salkun arvo vähintään kaksi kertaa aloittaaksesi seurannan.")
    ...
    with tab1:
        ...
        if selected_portfolio_name != "Uusi salkku":
            if st.button(f"Tarkastele salkkua '{selected_portfolio_name}'"):
                ...
                st.markdown("---")
                st.subheader("Salkun arvon tallennus")
                if st.button("Tallenna salkun tämänhetkinen arvo"):
                    history = load_portfolio_history()
                    if selected_portfolio_name not in history:
                        history[selected_portfolio_name] = []
                    
                    today = date.today().isoformat()
                    # Tarkista, onko arvo jo tallennettu tälle päivälle
                    existing_dates = [item[0] for item in history[selected_portfolio_name]]
                    if today in existing_dates:
                        st.warning("Salkun arvo on jo tallennettu tälle päivälle.")
                    else:
                        history[selected_portfolio_name].append([today, total_current_value])
